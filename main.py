...
indivi_stat_group = [[], [], [], [], [], [], [], [], [], [], [], [], []]
hof_list = ["Derek Jeter", "Larry Walker", "Edgar Martinez", "Vladimir Guerrero", "Chipper Jones", "Ivan Rodriguez",
            "Ken Griffey Jr.",
            "Mike Piazza", "Jim Thome", "Tim Raines", "Jeff Bagwell", "Craig Biggio", "Barry Larkin", "Roberto Alomar",
            "Andrew Dawson", "Ricky Henderson", "Jim Rice", "Tony Gwynn", "Cal Ripken Jr", "Wade Boggs",
            "Ryan Sandberg",
...
def hof_database_creator():
    ...
    for name in hof_list:
        ...
        for x in second_results:
            for p, stat_name, indiv_stat, x in zip(x.findAll('p'), second_grouping_totals, indivi_stat_group,
                                                   range(5, 8)):
                temp = float(p.text)
                second_grouping_totals[stat_name] += temp
                indivi_stat_group[x].append(temp)
        for x in third_results:
            for p, stat_name, indiv_stat, x in zip(x.findAll('p'), third_grouping_totals, indivi_stat_group,
                                                   range(8, 12)):
                temp = float(p.text)
...
def points_determination():
    # Calculates
    total_points = 0
    for player_stat_name_key, group_stat_name_key, st_dev_key in zip(first_player_totals, first_grouping_totals,
                                                                     first_grouping_stdev):
    ...
    for player_stat_name_key, group_stat_name_key, st_dev_key in zip(second_player_totals, second_grouping_totals,
                                                                     second_grouping_stdev):
    ...
    for player_stat_name_key, group_stat_name_key, st_dev_key in zip(third_player_totals, third_grouping_totals,
                                                                     third_grouping_stdev):
